...
- NEVER use `git push -f` or `--force`
- NEVER delete branches with `git branch -D` (use `git branch -d` after merge)
- NEVER use `git reset --hard`
- Only push to feature branches (e.g., `ralph/*`, `feature/*`, `bugfix/*`)
...
- ALWAYS run `black .` before committing (Python formatting)
- NEVER ignore linter warnings without documenting why
- NEVER remove `# type: ignore` comments without verifying on the TARGET Python version — different Python versions have different type stubs, and a comment that seems unnecessary on one version may be required on another
...
<!-- Container Safety — uncomment this section when running Ralph inside Docker
...
- Container runs as non-root user — cannot modify system files
...
End of Container Safety section -->
...
<!-- Example:
## GUI / Rendering
- Matplotlib blitting: NEVER recreate artists per frame. Initialize line artist once
  (e.g. `self._line, = ax.plot([], [])` with `animated=True`), then use `set_data()` +
  `ax.draw_artist()` + `fig.canvas.blit()` in the update loop. `draw_idle()` is a full
  redraw and defeats blitting.
- When a bead specifies a performance technique, tests must verify the technique is
  actually used (e.g., assert artist is reused, assert `draw_idle` is not called).
-->

...
Standard Ralph loop prompt with MCP-based visual validation for GUI projects.
Use this when your project has a graphical interface and beads that require
visual verification (screenshots) to confirm acceptance criteria.
...
Run your project's verification script (e.g. `bash verify.sh`).
...
If the bead has no visual criteria, skip to Step 6.
...
Kill the app process. Wait 2 seconds for cleanup.
...
- Run verification before closing — no exceptions
- Never skip failing tests — fix them or `<verify-fail>`
- Commit after each completed task, before stopping
- **3-strike rule**: if verification fails 3 times on the same issue, commit WIP, record what you tried, and bail with `<verify-fail>` — the next iteration gets a fresh context window
- NEVER modify verification scripts unless the task explicitly requires it
- NEVER modify build/tool config unless the task explicitly requires it
